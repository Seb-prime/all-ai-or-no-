...
with right:
   ...
   # ── Recent threats ─────────────────────────────────────────────────────────
    ...
    threats = backend.get_threats()  # calls ueba.get_threats()

    if not threats:
        st.markdown(
            '<div class="fwx-card" style="color:#4a7fa5;font-family:\'Share Tech Mono\',monospace;'
            'font-size:0.75rem;letter-spacing:0.1em;">NO THREATS RECORDED</div>',
            unsafe_allow_html=True,
        )
    else:
        for t in threats[:10]:   # show latest 10
            reasons = " · ".join(t.get("threat_reasons", ["anomaly detected"]))
            st.markdown(
                f'<div class="threat-row">'
                f'[{t.get("type","?").upper()}] &nbsp; USER: {t.get("user","?")} &nbsp;·&nbsp; '
                f'IP: {t.get("ip_address","?")} &nbsp;·&nbsp; '
